...
print(maxi_int)
```
The 5-minute interval with the maximum activity is the `r maxi_int[1]`th interval, which corresponds to 8:35 to 8:40 am. 
...
nb<-nrow(missing)
```
There are `r nb` missing values in the steps variable.
...
```
With the filled data, on average, `r formatC(full_mean_step,digits=2, format="f")` steps were taken every day. The median value is `r formatC(full_med,digits=2, format="f")`.    
Comments: !!!!  
